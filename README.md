...
The Context and Two Tier Permission packages use version `0.5.0-beta.7`. The Agents package uses version `0.5.0-beta.8`. The package payload places each module manifest, module assembly, dependency graph, and package-local dependency assemblies under `sharpclaw\`. The package metadata uses this repository and the `AGPL-3.0-only` license.
