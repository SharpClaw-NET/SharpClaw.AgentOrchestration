...
The Contracts and Context packages use version `0.5.0-beta.14`. The Two Tier Permission and Agents packages use version `0.5.0-beta.15`. The package payload places each module manifest, module assembly, dependency graph, and package-local dependency assemblies under `sharpclaw\`. The package metadata uses this repository and the `AGPL-3.0-only` license.
